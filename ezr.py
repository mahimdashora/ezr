...
def loglike(self:DATA, r:row, nall:int, nh:int, noise_strength: float) -> float:
  prior = (len(self.rows) + the.k) / (nall + the.k*nh)
  # likes = [c.like(r[c.at], prior) for c in self.cols.x if r[c.at] != "?"]
  # Adding noise to each feature value in the row for likelihood calculation
  likes = []
  for c in self.cols.x:
    if r[c.at] != "?":
      # Perturb the feature value with random noise before likelihood calculation
      noisy_value = r[c.at] + np.random.normal(0, noise_strength)
      likes.append(c.like(noisy_value, prior))
...
def activeLearning(self:DATA, score=lambda B,R: B-R, generate=None, faster=True ):
  ...
  def guess(todo:rows, done:rows) -> rows:
    ...
    if generate:
      ...
      key = lambda r: score(best.loglike(r, len(done), 2, noise_strength = 0.1), rest.loglike(r, len(done), 2, noise_strength = 0.1))
      # key = lambda r: score(best.loglike(r, len(done), 2), rest.loglike(r, len(done), 2)) \
      #   - self.measure_noise_stability(r, noise_strength=0.1)  # Adding noise stability penalty
  ...
  # Call accuracy calculation here after labeling
  accuracy = self.calculate_accuracy(done)
  print(f"Active Learning Accuracy: {accuracy}%")
